...
push `ele-autopilot/v*` tag → 根仓库 `.github/workflows/autopilot.yml` 构建 + 部署到 Cloudflare Workers. v0.3.0 起底层从 Node tarball 迁移到 Workers.
...
- `package.json#version` 与 tag 一致 (tag 格式 `ele-autopilot/vX.Y.Z`, version 不含 namespace 和 `v`). Actions 校验 tag 末段去 `v` 后等于 version, 不一致直接 fail.
...
```bash
TAG=ele-autopilot/vX.Y.Z
git add .
git commit -m "release(autopilot): vX.Y.Z"
git tag -a "$TAG" -m "$TAG"
