...
push `v*` tag → Actions 构建 + 部署到 Cloudflare Workers. v0.3.0 起底层从 Node tarball 迁移到 Workers.
...
- `package.json#version` 与 tag 一致 (tag 含 `v`, version 不含). Actions `Verify tag matches package.json version` 校验, 不一致直接 fail.
