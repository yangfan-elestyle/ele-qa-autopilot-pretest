...
- CF Access JWT 校验在 `cf-access-jwt-assertion` header 缺失时, 回退读 `CF_Authorization` cookie, 与 ele-autopilot 参考实现对齐. 当前所有非 Bypass 路径都走 Allow App, header 必有; 此变更是纵深防御补强, 用户层无感知.
...
- 首页顶栏移除装饰性的「服务运行中」标记 (无实际健康检测, 仅误导).
