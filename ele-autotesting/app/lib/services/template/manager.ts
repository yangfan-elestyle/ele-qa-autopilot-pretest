...
export class TemplateManager implements ITemplateManager {
  ...
  async listTemplates(): Promise<Template[]> {
    const [builtinTemplates, userTemplates] = await Promise.all([this.getBuiltinTemplates(), this.getUserTemplates()])
  ...
  private async getBuiltinTemplates(): Promise<Record<string, Template>> {
    const templateSet = this.staticLoader.getDefaultTemplates()
  ...
  private async persistUserTemplates(templates: Template[]): Promise<void> {
    try {
      await this.storageProvider.setItem(CORE_SERVICE_KEYS.USER_TEMPLATES, JSON.stringify(templates))
    } catch (error) {
  ...
  async exportData(): Promise<Template[]> {
    try {
      ...
      throw new ImportExportError('Failed to export template data', await this.getDataType(), error as Error)
    }
  }
  ...
  async importData(data: any): Promise<void> {
    ...
    const userTemplateIds = existingTemplates.filter((template) => !template.isBuiltin).map((template) => template.id)
      ...
      try {
        ...
        if (!this.validateSingleTemplate(template)) {
          ...
          failedTemplates.push({
            template,
            error: new Error('Invalid template configuration'),
          })
          continue
        }
 ...
 * @returns 模板管理器实例
 */
export function createTemplateManager(storageProvider: IStorageProvider): TemplateManager {
  return new TemplateManager(storageProvider)
}
