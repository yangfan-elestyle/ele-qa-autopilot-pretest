...
function isDangerousParameter(paramName: string): boolean {
  return DANGEROUS_PARAMS.some((dangerous) => paramName.toLowerCase().includes(dangerous.toLowerCase()))
}

/**
 * 验证llmParams中的已知参数（增强安全版本）
 */
export function validateLLMParams(llmParams: Record<string, any> | undefined, provider: string): LLMValidationResult {
  ...
  // 获取该提供商支持的参数定义
  const supportedParams = advancedParameterDefinitions.filter((def) => def.appliesToProviders.includes(provider))
...
function validateSingleParameter(value: any, def: AdvancedParameterDefinition): { isValid: boolean; message?: string } {
