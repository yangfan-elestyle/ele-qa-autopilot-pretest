import { IHistoryManager, PromptRecord, PromptRecordChain, ContentType } from './types'
...
export class HistoryManager implements IHistoryManager {
  ...
  private readonly maxRecords = 1000 // Keep up to 1000 records in history
  ...
  async addRecord(record: PromptRecord): Promise<void> {
    try {
      ...
      await this.storage.updateData<PromptRecord[]>(this.storageKey, (existingRecords: PromptRecord[] | null) => {
  ...
  private validateRecord(record: PromptRecord): void {
    ...
    if (!record.contentType) errors.push('Content type is required')
    ...
    if ((record.contentType === 'prompt_url' || record.contentType === 'prompt_figma') && !record.contents) {
      errors.push('URL is required when content type is url or figma')
    }
    if (record.contentType === 'prompt_image' && !record.contents) {
      errors.push('Image is required when content type is image')
    }
    if (record.contentType === 'prompt_file' && !record.contents) {
      errors.push('File description is required when content type is file')
    }
  ...
  async addIteration(params: {
    ...
    const record: PromptRecord = {
      ...
      type: 'IterateRecordType',
      contentType: params.contentType,
      contentMark: params.contentMark,
  ...
  async exportData(): Promise<PromptRecord[]> {
    try {
      ...
      throw new ImportExportError('Failed to export history data', await this.getDataType(), error as Error)
    }
  }
