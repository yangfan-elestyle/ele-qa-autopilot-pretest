...
import { renderSvgToPng } from "~/server/services/svgRenderer";
import { bytesToBase64 } from "~/server/utils/fileDownload";
import type { Route } from "./+types/figma-parse";
...
const parseFigmaUrl = (value: string) => {
  let url: URL;
  try { url = new URL(value); } catch { throw new Error("Invalid Figma URL format"); }
  if (!url.hostname.endsWith("figma.com")) throw new Error("Invalid Figma URL host");
  const segments = url.pathname.split("/").filter(Boolean);
  if (segments.length < 3) throw new Error("Invalid Figma URL path");
  const nodeParam = url.searchParams.get("node-id");
  if (!nodeParam) throw new Error("Missing node-id in Figma URL");
  return {
    fileType: segments[0],
    fileKey: segments[1],
...
const fetchSvgAndConvertToBase64Png = async (url: string, width?: number, height?: number) => {
  ...
  if (!response.ok) throw new Error(`Failed to download SVG: HTTP ${response.status}`);
...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  if (!token) return Response.json({ error: "Missing Figma token" }, { status: 400 });

  let parsed;
  try { parsed = parseFigmaUrl(rawUrl); }
  catch (err: any) { return Response.json({ error: err?.message || "Invalid Figma URL" }, { status: 400 }); }
  ...
  try {
    ...
    if (!fileResponse.ok) {
      ...
      return Response.json(
        ...
        { status: fileResponse.status === 404 ? 404 : 400 },
      );
    }
    fileJson = (await fileResponse.json()) as FigmaFileResponse;
  } catch (err: any) {
    console.error("Figma file request error:", err?.message || err);
    return Response.json({ error: "Failed to fetch Figma file", details: err?.message || String(err) }, { status: 502 });
  }

  const targetNode = findNodeById(fileJson.document, parsed.nodeId);
  if (!targetNode) {
    return Response.json({ error: "Node not found in Figma file", details: { nodeId: parsed.nodeId } }, { status: 404 });
  }
  ...
  try {
    const params = new URLSearchParams({ ids: childNodes.map((child) => child.id).join(","), format: "svg" });
    ...
    if (!imagesResponse.ok) {
      ...
      return Response.json(
        ...
        { status: imagesResponse.status === 404 ? 404 : 400 },
      );
    }
    imagesJson = (await imagesResponse.json()) as FigmaImagesResponse;
  } catch (err: any) {
    console.error("Figma images request error:", err?.message || err);
    return Response.json({ error: "Failed to fetch Figma images", details: err?.message || String(err) }, { status: 502 });
  }

  if (imagesJson.err) return Response.json({ error: "Figma images API responded with error", details: imagesJson.err }, { status: 400 });
  ...
  const analyses: Array<{ nodeId: string; imageUrl: string; size: number; success: boolean; data?: unknown; error?: string }> = [];

  for (const item of selectedImages) {
    try {
      const { base64, mime } = await fetchSvgAndConvertToBase64Png(item.imageUrl, item.width, item.height);
      const result = await analyzeImage({ config, prompt, imageBase64: base64, mime });
