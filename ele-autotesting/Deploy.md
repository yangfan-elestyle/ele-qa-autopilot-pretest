...
AI 改完代码主动执行. push `ele-autotesting/v*` tag 触发根仓库 `.github/workflows/autotesting.yml`, CI 跑 D1 migrations + `wrangler deploy`.
...
- `package.json#version` 与 tag 一致 (tag 格式 `ele-autotesting/vX.Y.Z`, version 不含 namespace 和 `v`). Actions 第一步校验 tag 末段去 `v` 后等于 version, 不一致直接 fail.
...
```bash
TAG=ele-autotesting/vX.Y.Z
git add .
git commit -m "release(autotesting): vX.Y.Z"
git tag -a "$TAG" -m "$TAG"
git push yangfan-elestyle main "$TAG"
```
...
git tag -d "$TAG"
git push yangfan-elestyle ":refs/tags/$TAG"
git tag -a "$TAG" -m "$TAG"
git push --force-with-lease yangfan-elestyle main
git push yangfan-elestyle "$TAG"
```
