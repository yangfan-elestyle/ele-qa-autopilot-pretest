...
AI 改完代码主动执行. push `v*` tag 触发 `.github/workflows/deploy-cf.yml`, CI 跑 D1 migrations + `wrangler deploy`.
...
- `package.json#version` 与 tag 一致 (tag 含 `v`, version 不含). Actions 第一步会校验, 不一致直接 fail.
...
push tag 后 GitHub Actions `deploy-cf` 自动跑. push `main` 不再触发 CI, 但仍要推, 保持远端分支与 tag 一致.
