...
- 「同步到云」现在会一并保存生成当时的完整上下文 — 输入区原始提示词、优化结果、已添加的【模板 / 上下文】数据源、选中的模块、所用生成模型与优化模式. 历史抽屉「使用」时这些设置会一次性还原回界面, 不再只剩生成结果裸文本.
- 云端用例快照的标题改为用「用户原始提示词」(取前 80 字), 比此前的「第一个用例名 / N 条用例」更容易识别历史条目.

### Added

- 「内容生成」结果工具栏新增两个图标按钮 (与「导出 Excel / 复制内容 / 全屏查看」并排):
  - **同步到云**: 默认不持久化, 点击后把当前生成结果原文与元信息存到 Cloudflare D1, 支持跨设备 / 跨会话还原.
  - **发送到联动**: 一键打开顶部「联动」面板并把当前生成结果送进去, 省去手动点顶部「联动」入口的步骤.
- 「历史」抽屉拆分子 Tab:「上下文历史」(原有提示词历史) 与「AutoTest 用例历史」(云端同步的用例快照), 后者支持「使用」一键还原到生成结果区, 也可单条删除或整体清空.
- OutputDisplay 工具栏新增 `extra-actions` 插槽, 父组件可注入自定义图标按钮, 不污染基础组件 (本次「同步到云」与「发送到联动」即用此插槽).
