...
// Worker 单实例 128MB 内存, c.req.json() 会把整个 body 读到 memory.
// 没有 size guard 时, 攻击者上传巨型 markdown (例如 50MB+) 直接把 Worker 拉
// 到 OOM 502, 业务挂掉. 2MB 文本对实际文档 (Confluence 单页, 设计稿导出 md)
