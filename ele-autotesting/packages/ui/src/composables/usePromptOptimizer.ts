...
import type { ContextConfig } from './usePromptHistory'
 ...
 * @param contextConfig 提示词类型数据
 * @returns 提示词优化器接口
 */
export function usePromptOptimizer(
  services: Ref<AppServices | null>,
  contextConfig: Ref<ContextConfig>,
  ...
  const state = reactive({
    ...
    isOptimizing: false,
    isIterating: false,
    selectedMainTemplate: null as Template | null,
    selectedIterationTemplate: null as Template | null,
    currentChainId: '',
  ...
  state.handleOptimizePrompt = async () => {
    ...
    // 根据优化模式选择对应的模板
    const currentTemplate = state.selectedMainTemplate

    if (!currentTemplate) {
      toast.error('请先选择优化模版')
      return
    }

    if (!optimizeModel.value) {
      toast.error('请先选择优化模型')
      return
    }
        ...
        onComplete: async () => {
          ...
          try {
            ...
            const recordData = {
              ...
              type: (optimizationMode.value === 'context' ? 'ContextOptimizeRecordType' : 'VerifyOptimizeRecordType') as
                | 'ContextOptimizeRecordType'
                | 'VerifyOptimizeRecordType',
              contentType: contextConfig.value.contentType,
              contentMark: contextConfig.value.contentMark,
              contents: contextConfig.value.contents,
              modelKey: optimizeModel.value,
              templateId: currentTemplate.id,
              timestamp: Date.now(),
              metadata: {
  ...
  const iterateImpl = async ({
    ...
    if (!state.selectedIterationTemplate) {
    ...
    try {
      await promptService.value!.iteratePromptStream(
          ...
          onComplete: async (_response: unknown) => {
            ...
            try {
              ...
              const iterationData = {
                ...
                modelKey: optimizeModel.value,
                templateId: state.selectedIterationTemplate.id,
              }
        ...
        state.selectedIterationTemplate.id,
      )
    } catch (error) {
      console.error('[Iterate] 迭代失败:', error)
      toast.error('迭代失败')
  ...
  // 统一暴露：如果有迭代参数则走迭代，否则执行首次优化
  state.handleIteratePrompt = async (payload?: { originalPrompt: string; optimizedPrompt: string; iterateInput: string }) => {
    if (payload && typeof payload.originalPrompt === 'string' && typeof payload.optimizedPrompt === 'string' && typeof payload.iterateInput === 'string') {
      return iterateImpl(payload)
    }
    // 首次执行：保持原有逻辑
    return state.handleOptimizePrompt()
  }
