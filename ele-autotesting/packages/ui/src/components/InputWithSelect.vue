<template>
  <div ref="componentRef" class="relative">
    ...
    <!-- Dropdown Menu -->
    <div v-if="isOpen" class="absolute z-10 mt-1 w-full rounded-md theme-dropdown shadow-lg max-h-60 overflow-auto">
      ...
      <ul v-else class="py-1">
        <li
          ...
          :class="['theme-dropdown-item cursor-pointer px-4 py-2', modelValue === option.value ? 'theme-dropdown-item-active' : 'theme-dropdown-item-inactive']"
        >
          {{ option.label }}
        </li>
      </ul>
...
const filteredOptions = computed(() => {
  ...
  return props.options.filter(
    (option) => option.label.toLowerCase().includes(searchText.value.toLowerCase()) || option.value.toLowerCase().includes(searchText.value.toLowerCase()),
  )
})
