...
class PromptOptimizerDB extends Dexie {
  ...
  constructor() {
    ...
    // v1: 原始 schema（保留以维持升级链路）
    this.version(1).stores({
      storage: 'key, value, timestamp',
    })

    // v2: 移除 value 索引（value 存储大型 JSON，索引会导致 Chrome 崩溃）
...
export class DexieStorageProvider implements IStorageProvider {
  ...
  constructor() {
    ...
    this.dbOpened = this.db
      ...
      .catch(async (error) => {
        ...
        // 数据库可能已损坏（如 value 索引异常），尝试删除后重建
        console.warn('Attempting database recovery: delete and recreate...')
        try {
          this.db.close()
          await Dexie.delete('PromptOptimizerDB')
          this.db = new PromptOptimizerDB()
          await this.db.open()
          console.warn('Database recreated successfully. Previous data was lost due to corruption.')
        } catch (retryError) {
   ...
   * 带重试机制的原子更新
   */
  private async _performAtomicUpdateWithRetry<T>(key: string, updateFn: (currentValue: T | null) => T, maxRetries: number = 3): Promise<void> {
   ...
   * 简单更新（降级方案）
   */
  private async _performSimpleUpdate<T>(key: string, updateFn: (currentValue: T | null) => T): Promise<void> {
    try {
      // 读取当前值
   ...
   * 执行原子更新
   */
  private async _performAtomicUpdate<T>(key: string, updateFn: (currentValue: T | null) => T): Promise<void> {
    try {
      // 使用更安全的事务处理方式
