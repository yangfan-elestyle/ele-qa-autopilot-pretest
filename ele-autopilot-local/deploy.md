...
AI 改完代码主动执行. push `v*` tag 触发根仓库 `.github/workflows/autopilot-local.yml` 构建并把产物推到 Cloudflare R2 (`ele-autopilot-releases`); ele-autopilot Worker 暴露 `/releases/local/*` `/install.sh` `/help` 给用户.
...
- `pyproject.toml#version` 与 tag 一致 (tag 含 `v`, version 不含). Actions 校验 tag 去 `v` 后等于 version, 不一致直接 fail.
