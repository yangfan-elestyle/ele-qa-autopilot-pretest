...
内网单机 docker-compose 承接 gateway + 三业务 + markitdown. 迁移背景与决策见 [../plans/20260713-001/feature.md](../plans/20260713-001/feature.md).
...
```bash
cd deploy
cp .env.example .env          # 填 LLM/Confluence 凭据等 (无对象存储密码要改)
...
2. **`.env`**: 填 LLM / Confluence 凭据; 确认 `METERSPHERE_URL=https://qa.elepay.link` 内网可达; agentic-loop 迁移后填 `AGENTIC_LOOP_URL`.
3. **数据迁移** (从 CF 导出): D1 → `sqlite3 /var/lib/docker/volumes/deploy_autopilot_data/_data/autopilot.db < dump.sql` (含 `settings.llm_api_key`); autotesting 同理. (库为空时 server 首启自动建表, 无历史数据可跳过.) 截图为可再生任务产物, 无需迁.
